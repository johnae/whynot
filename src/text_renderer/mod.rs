...
impl TextRendererFactory {
    ...
    pub async fn create_converter(config: &TextRendererConfig) -> TextRendererResult<Box<dyn HtmlToTextConverter>> {
        match config.converter_type {
            ConverterType::Builtin => {
                Ok(Box::new(builtin::BuiltinConverter::new(config.clone())))
            },
            ConverterType::External => {
                let command = config.external_tool_command.as_ref()
                    .ok_or_else(|| TextRendererError::ConfigError(
                        "External tool command not specified".to_string()
                    ))?;
                
                let converter = external::ExternalToolConverter::new(command.clone(), config.clone());
            ...
            ConverterType::Auto => {
                ...
                if let Some(command) = &config.external_tool_command {
                    let external_converter = external::ExternalToolConverter::new(command.clone(), config.clone());
