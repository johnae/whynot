...
use super::{HtmlToTextConverter, TextRendererConfig, TextRendererResult, TextRendererError};
...
impl ExternalToolConverter {
    ...
    fn parse_command(&self) -> (String, Vec<String>) {
        let parts: Vec<String> = self.command
            .split_whitespace()
            .map(|s| s.to_string())
            .collect();
    ...
    async fn convert_with_tool(&self, html: &str) -> TextRendererResult<String> {
        ...
        let mut child = Command::new(&cmd)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|e| TextRendererError::ExternalToolError(
                format!("Failed to start command '{}': {}", cmd, e)
            ))?;
        ...
        if !output.status.success() {
            ...
            return Err(TextRendererError::ExternalToolError(
                format!("Command failed with status {}: {}", output.status, stderr)
            ));
        }
...
mod tests {
    ...
    async fn test_convert_with_invalid_command() {
        ...
        assert!(matches!(result, Err(TextRendererError::ExternalToolError(_))));
    }
