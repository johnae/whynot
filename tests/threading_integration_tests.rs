...
async fn test_self_reply_threading() {
    ...
    // Get the thread
    let search_results = client.search("subject:\"Important Discussion\"").await.unwrap();
    ...
    let reply_msg = updated_messages.iter()
        .find(|m| m.headers.subject.starts_with("Re:"))
        .expect("Should find reply message");
    
    assert_eq!(reply_msg.headers.to, "recipient@company.com", "Reply should go to original recipient, not self");
    assert_eq!(reply_msg.headers.from, "<test@example.com>", "Reply should be from user");
}
...
async fn test_normal_reply_threading() {
    ...
    assert_eq!(reply_msg.headers.to, "sender@company.com", "Reply should go to original sender");
    assert_eq!(reply_msg.headers.from, "<test@example.com>", "Reply should be from user");
}
...
async fn test_reply_all_threading() {
    ...
    assert_eq!(updated_messages.len(), 2, "Reply-all should be in same thread");
    ...
    assert_eq!(reply_msg.headers.to, "manager@company.com", "Reply should go to original sender");
    assert_eq!(reply_msg.headers.from, "<test@example.com>", "Reply should be from user");
    
    // Check CC header if available
    if let Some(cc) = reply_msg.headers.additional.get("cc") {
        assert!(cc.contains("colleague1@company.com"), "CC should include colleague1");
        assert!(cc.contains("colleague2@company.com"), "CC should include colleague2");
    }
}
...
async fn test_threading_with_missing_message_id() {
    ...
    let search_results = client.search("subject:\"Missing Message ID Test\"").await.unwrap();
    assert_eq!(search_results.len(), 1);
    let thread_id = search_results[0].thread_id();
    ...
    if let Some(reply) = reply_msg {
        assert_eq!(reply.headers.to, "sender@example.com", "Reply should go to original sender");
    }
}
...
async fn test_references_header_chain() {
    ...
    let _root_msg = messages.iter().find(|m| !m.headers.subject.starts_with("Re:")).unwrap();
    let replies: Vec<_> = messages.iter().filter(|m| m.headers.subject.starts_with("Re:")).collect();
    ...
    for reply in replies {
        if let Some(references) = reply.headers.additional.get("references") {
            assert!(references.contains(root_id), "References should contain root message ID");
        }
    }
}
...
async fn test_thread_search_finds_replies() {
    ...
    assert_eq!(messages.len(), 2, "Thread should contain both original and reply");
    
    // Verify both messages are present
    let subjects: Vec<_> = messages.iter().map(|m| &m.headers.subject).collect();
    assert!(subjects.contains(&&"Searchable Thread".to_string()));
    assert!(subjects.contains(&&"Re: Searchable Thread".to_string()));
}
...
async fn test_broken_threading_fallback() {
    ...
    assert!(!search_results.is_empty(), "Should find messages even with broken threading");
    
    // Each message should be accessible
    for result in search_results {
        let thread = client.show(&format!("thread:{}", result.thread_id())).await.unwrap();
        let messages = thread.get_messages();
        assert!(!messages.is_empty(), "Each thread should have at least one message");
    }
}