use whynot::{
    ...
    error::NotmuchError,
    mail_sender::{MailSender, ComposableMessage},
    thread::Message,
};
...
impl MailSender for MockMailSender {
    ...
    async fn reply(&self, _original: &Message, _reply: ComposableMessage, _reply_all: bool) -> Result<String, NotmuchError> {
        Ok("mock-reply-id".to_string())
    }

    async fn forward(&self, _original: &Message, _forward: ComposableMessage) -> Result<String, NotmuchError> {
...
async fn test_compose_multiline_body_input() {
    ...
    let mut app = App::new(client as std::sync::Arc<dyn NotmuchClient>, mail_sender).await.unwrap();
